...
### Synchronous Binding Mutations

**Problem**: Mutating `appState` or calling `dispatch()` synchronously inside a `Binding` setter causes SwiftUI runtime warnings:

> "Publishing changes from within view updates is not allowed, this will cause undefined behavior."

This happens because SwiftUI is already in the middle of a view update when the `Binding` setter runs. Synchronous state mutations trigger another update cycle while the first one is still in progress.

**BAD - Synchronous mutation in Binding setter:**
```swift
Binding(
  get: { appState.value },
  set: { appState.dispatch(.setValue($0)) }  // Triggers during view update!
)
```

**GOOD - Deferred mutation via Task:**
```swift
Binding(
  get: { appState.value },
  set: { newValue in
...
**Why this works**: `Task { @MainActor in }` schedules the mutation for the next run loop iteration, after the current view update completes. This is the SwiftUI-idiomatic way to handle side effects in `Binding` setters.

**Enforcement**:
- CI check: `./scripts/check_binding_patterns.sh` runs in CI pipeline
- SwiftLint: `binding_sync_mutation` custom rule in `.swiftlint.yml`
- Tests: `BindingPatternTests.swift` scans source files for violations
