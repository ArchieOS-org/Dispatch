...
final class FilterUITests: XCTestCase {
  ...
  ///
  /// - Note: This test is disabled in CI due to timing flakiness. The filter button
  ///   visibility depends on `LensState.currentScreen` being updated after navigation,
  ///   which has race conditions in CI environments. See: DISPATCH-XXX
  @MainActor
  func testAudienceFilterCycle() throws {
    // TODO: Fix underlying race condition between navigation and LensState updates
    // The filter button only appears when lensState.currentScreen == .myWorkspace,
    // which is set asynchronously after navigation completes. CI runners are too
    // slow/variable for the current 5-second timeouts.
    throw XCTSkip("Flaky on CI - filter button visibility has race condition with navigation state")
