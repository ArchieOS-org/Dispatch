...
extension Listing: RealtimeSyncable {
  ...
  func markPending() {
    ...
    #if DEBUG
    if suppressed {
      print("[SYNC_DEBUG] markPending SUPPRESSED on Listing id=\(id) - isSyncing=\(isSyncing)")
    } else {
      print("[SYNC_DEBUG] markPending EXECUTED on Listing id=\(id) - isSyncing=\(isSyncing)")
