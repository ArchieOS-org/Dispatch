...
extension TaskItem: RealtimeSyncable {
  ...
  func markPending() {
    ...
    #if DEBUG
    if suppressed {
      print("[SYNC_DEBUG] markPending SUPPRESSED on TaskItem id=\(id) - isSyncing=\(isSyncing)")
    } else {
      print("[SYNC_DEBUG] markPending EXECUTED on TaskItem id=\(id) - isSyncing=\(isSyncing)")
