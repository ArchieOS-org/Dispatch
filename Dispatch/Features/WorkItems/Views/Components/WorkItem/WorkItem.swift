...
enum WorkItem: Identifiable {
  ...
  var statusText: String {
    snapshot.statusRawValue.capitalized
  }
  ...
  var notes: [Note] {
    switch self {
    case .task(let task, _): task.notes
    case .activity(let activity, _): activity.notes
    }
  }
  ...
  var subtasks: [Subtask] {
    switch self {
    case .task(let task, _): task.subtasks
    case .activity(let activity, _): activity.subtasks
    }
  }
  ...
  var listing: Listing? {
    switch self {
    case .task(let task, _): task.listing
    case .activity(let activity, _): activity.listing
    }
  }
  ...
  private var snapshot: WorkItemSnapshot {
    switch self {
    case .task(_, let snapshot): snapshot
    case .activity(_, let snapshot): snapshot
    }
  }
